...
| `GP2` | `LIFT_HOME` normally-open microswitch to local `TOOL_GND`. Firmware uses `INPUT_PULLUP`; the raw pin is HIGH when released and LOW when pressed. Telemetry reports the semantic state as `lift_home=0` released and `lift_home=1` pressed/home. The current implementation reports this in native-USB and GP20/GP21 service-UART telemetry only; it does not yet control motor motion. |
| `GP4` | DRV8833 `IN1` |
| `GP5` | DRV8833 `IN2` |
...
The CS1238 moving average is retained as telemetry in this mode; it does not
pretend to detect paper contact or silently authorize motion. The measured
20 x 5 ms manual UP round trip (about 1.75 mm clearance) and matching manual
DOWN paper-resistance check are the current bench evidence for the staged
duration, not a replacement for the normal force-control calibration profile.
...
| [`e09e_cs1238_pen_scale_pulse/e09e_cs1238_pen_scale_pulse.ino`](e09e_cs1238_pen_scale_pulse/e09e_cs1238_pen_scale_pulse.ino) | Dedicated installed-pen/kitchen-scale direction check after E-09C. Its runtime service link is UART1 (`GP20` TX / `GP21` RX) through the existing 3.3 V USB-to-TTL adapter, with adapter VCC disconnected; USB-C is flash-only while the external toolhead rail is disconnected. Arduino IDE Serial Monitor at 115200 supports immediate `t`, `a`, `d`, `u`, `r`, `x`, `[`, `]`, and `?` shortcuts: tare, arm, individual supervised 5–100 ms N20 pulses, raw read, stop, duration adjustment in 5 ms steps, and help. It checks `ULT`, sleeps after every pulse, and blocks lift when GP2 is pressed. It has no M3/M5, GP27, continuous drive, or automatic force seeking. | CS123x by FMazz97, 1.1.0 |
| [`e09f_cs1238_guarded_force_hold/e09f_cs1238_guarded_force_hold.ino`](e09f_cs1238_guarded_force_hold/e09f_cs1238_guarded_force_hold.ino) | Temporary E-09F supervised automatic test using the E-09C precision-weight profile. After explicit tare/arm, `s` applies at most 30 5 ms corrections per direction with 500 ms settling to seek/hold the 40–60 g raw band for five seconds; `c` releases to the 3 g raw band then applies one explicit 100 ms candidate air-gap pulse. It has 30 s timeout, 70 g raw hard limit, ULT/GP2 checks, and sleeps between pulses. It does not configure GP29/M3/M5, GP27, or magnetic logic. | CS123x by FMazz97, 1.1.0 |
