...
| `GP28` / `A2` | Two-phase arm input from PC817C U2. An assertion pulls GP28 LOW: first arm requests readiness ACK, release clears it, second arm exposes threshold state on GP27. During normal print (`DISARMED`, pen not at GP2) the same assertion requests a full retract to the GP2 lift-home switch for end-of-print paper clearance. |
...
| [`e09e_cs1238_pen_scale_pulse/e09e_cs1238_pen_scale_pulse.ino`](e09e_cs1238_pen_scale_pulse/e09e_cs1238_pen_scale_pulse.ino) | Dedicated installed-pen/kitchen-scale direction check after E-09C. Its runtime service link is UART1 (`GP20` TX / `GP21` RX) through the existing 3.3 V USB-to-TTL adapter, with adapter VCC disconnected; USB-C is flash-only while the external toolhead rail is disconnected. Arduino IDE Serial Monitor at 115200 supports immediate `t`, `a`, `d`, `u`, `r`, `x`, `s`, `[`, `]`, and `?` shortcuts: tare, arm, individual supervised 5–100 ms N20 pulses, raw read, stop, one 5 ms pulse plus a 2.5 s settle trace, duration adjustment in 5 ms steps, and help. It checks `ULT`, sleeps after every pulse, and blocks lift when GP2 is pressed. It has no M3/M5, GP27, continuous drive, or automatic force seeking. | CS123x by FMazz97, 1.1.0 |
