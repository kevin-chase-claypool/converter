...
| [`e09e_cs1238_pen_scale_pulse/e09e_cs1238_pen_scale_pulse.ino`](e09e_cs1238_pen_scale_pulse/e09e_cs1238_pen_scale_pulse.ino) | Dedicated installed-pen/kitchen-scale direction check after E-09C. Its runtime service link is UART1 (`GP20` TX / `GP21` RX) through the existing 3.3 V USB-to-TTL adapter, with adapter VCC disconnected; USB-C is flash-only while the external toolhead rail is disconnected. It accepts only explicit, supervised 10–100 ms N20 pulses; it requires tare plus `ARM`, checks `ULT`, sleeps after every pulse, blocks lift when GP2 is pressed, and retains raw CS1238 capture. It has no M3/M5, GP27, continuous drive, or automatic force seeking. The E-07D Windows app enables its pulse controls only after it reads this sketch's explicit status mode. | CS123x by FMazz97, 1.1.0; Windows app: pyserial and matplotlib |
