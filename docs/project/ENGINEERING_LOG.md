...
- Status: post-flash manual down/up test passed for this pen installation.
...
- Evidence: after manually setting approximate drawing preload, twenty guarded
  ...
  the staged mechanical clear value. The paper-pull check had noticeable but
  operator-judged acceptable resistance; twenty guarded `d` pulses returned
  the same practical resistance.
...
- Status: the first 5 ms load/unload cycle confirms the installed sign, working range, and clear return. It is not a replacement calibration.
...
- Result: raw moved negative relative to the `253,636 raw` clear tare, confirming the E-09C opposite-direction selection. The non-outlier 64.1–70.0 g trace has a local 4,717 raw/g slope, close to the 5,038.77 raw/g precision-mass result, and the 69.5–63.6 g unload segment overlaps it. The precision-mass fit remains the raw calibration authority; kitchen-scale hysteresis is controller-tuning evidence, not a reason to refit the slope. Ten milliseconds is too coarse near target; E-09E now provides 5 ms increments. The staged M5 air-gap candidate is reduced from 500 ms to 100 ms based only on observed motor speed; it remains gated by T-01H.
- Next action: use the accepted precision-weight raw profile for the bounded controller-dynamics and pen-clear tests; only verify that kitchen-scale force remains approximately 40–60 g.
...
Measured E07B force response, recorded in the E-14C lab note: from a clear
tare of 233439, one 20 ms down pulse reached 19.6 g at
`hx_delta=-349534`; one 20 ms up pulse then returned the scale to 0.0 g at
`hx_delta=-8217`. From a later clear tare of 218958, two 10 ms down pulses
...
- Status: resolved and hardware-verified.
...
- Verification: corrected Q5 completed with no subsequent `Home` state and
  stopped at `MPos:-232.013,-218.775`; a magnet beneath that TMAG endpoint
...
### 🟩 2026-09-11 - RP23CNC-SOFTWARE/HARDWARE/VERIFIED - Complete non-registering P112 survey

- Status: M-09 survey capture, final trim, and physical-center verification
  passed; registration remains separately locked.
- Category: rp23cnc-software, hardware, P112, A-axis, index-magnet, M-09,
  safety.
- Evidence: the second footprint captured `A8557.209..8662.959`; P112 reported
  ...
  `Pn:ZA`; no error, alarm, or G54 write occurred. The operator confirmed the
...
- Correction: the 7.23 mm assertion and 7.97 mm release measurements span the
  lead screw in its heat-set nut, not spring seats; their 0.74 mm difference is
  a lead-screw/nut position window. Free-spring solid height is 3.75 mm.
  Compression occurs only under upward pen/paper force, so solid height is a
  contact-force bound, not a retract-travel bound.
- Boundary: actual spring-seat lengths, true backstop margin, released-state
...
- Diagram clarification: the bench schematic now draws PC USB as the Pico
