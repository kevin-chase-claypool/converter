# Lab Note: 2026-09-23 - T-02 hysteresis relief four-cycle run
...
Four M3/M5 cycles completed with **no faults**: zero `FAULT_EVENT` lines.
...
- First-touch reference spread: 38,620 to 56,803 raw, **18,183 raw (about
  3.6 g)**.
...
urgent_relief_count=9 urgent_relief_ms=199   (cycles 1 through 4 snapshots)
```
...
read 199,658 raw, comfortably in band. Cycles 2 through 4 did not trigger the
relief at all.
...
The counters also confirm the earlier hunting report was not relief activity:
cycles 2 through 4 show `urgent_relief_count` unchanged at 9 and 199 ms, so the
relief never fired during them.

## Decisions and next action

Continue building repeat evidence toward the T-01H 30-cycle count. Watch
