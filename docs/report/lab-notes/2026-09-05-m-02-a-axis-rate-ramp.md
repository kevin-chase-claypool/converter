...
2. Increased the commanded `G1` feed in bounded steps from `F180` through
   `F480`, using equal positive and negative moves at each rate.
...
- The A-axis completed the tested rate steps at `F180`, `F240`, `F300`,
  `F360`, `F420`, and `F480` without reported stalling or jerking.
...
- The motor remained cool to the touch after the `F480` test.
...
- Dwell duration and an instrumented temperature measurement were not
  recorded.
- Disposition: **M-02 is in progress; `F480` is the highest A-axis rate
  validated so far under this test setup, with a recorded current of
  approximately `0.465 A`.**

## Difficulties and corrective actions

None encountered during the recorded `F180`-`F480` A-axis rate steps. The
earlier X-axis jerking was part of M-01 and was not observed in this A-axis
rate ramp.

## Interpretation

The A-axis has demonstrated clean, repeatable motion through `F480` for the
tested move pattern, with no observed position loss or heating. The measured
`0.465 A` supply current is well below the temporary 2 A limit. This supports
using `F480` as a currently tested rate, subject to the configured A-axis
maximum rate and the actual operating duty cycle. It is not yet evidence that
`F480` is the absolute motor or controller limit. The lack of quantitative
temperature and dwell data limits the strength of the thermal conclusion.
...
`F480` as the highest A-axis rate tested so far and continue M-02 with the X
and Y axes; a longer dwell or instrumented temperature check can be added if
