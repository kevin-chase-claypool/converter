...
|---|---|---|---|
| 2026-09-25 | T-01H clearance across print runs | Passed — behavioral across production runs (measured record open) | Several complete letter prints exercised hundreds of `M3`/`M5` cycles: the pen settled clear above the paper after every 57 ms clearance pulse, with no drag, marking, or stray contact. The cycle count far exceeds the 30-cycle requirement, so T-01H's behavioral criterion is met. Still open for the formal record: the pen-tip gap in millimetres and a force trace with `F_contact_on` / `F_release_off`. See [`2026-09-25-t-01h-clearance-confirmed-across-print-runs.md`](2026-09-25-t-01h-clearance-confirmed-across-print-runs.md). |
