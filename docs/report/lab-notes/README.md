...
|---|---|---|---|
| 2026-09-05 | M-02 in progress | In progress | A-axis rate ramp passed cleanly through the earlier `F500` ceiling (`0.485 A`, cool). `$103=4.44444` is corrected, and unloaded acceleration tests through `$123=6000` / `F80000` were reported smooth with exact mark returns; the F40000 test read `0.492 A` moving and `0.122 A` idle, and the motor remained cool to touch. Instrumented/repeated thermal, cable-strain, pen-load, and M-05 checks remain. |
