...
|---|---|---|---|
| 2026-09-25 | T-01H clearance across print runs | Passed — behavioral across production runs; gap measured, force trace open | Several complete letter prints exercised hundreds of `M3`/`M5` cycles: the pen settled clear above the paper after every 57 ms clearance pulse, with no drag, marking, or stray contact. The operator measured the pen-tip gap at about 1.75 mm. The cycle count far exceeds the 30-cycle requirement, so T-01H's behavioral criterion is met. Still open for the formal record: a force trace with `F_contact_on` / `F_release_off` and the release debounce. See [`2026-09-25-t-01h-clearance-confirmed-across-print-runs.md`](2026-09-25-t-01h-clearance-confirmed-across-print-runs.md). |
| 2026-09-24 | E-18 / M-08 / M-09 first integrated-firmware P113 registration | Passed — pen tip confirmed centered over the magnet | The production toolhead build with `MAGNETIC_CALIBRATION_VALID = true` ran `G65 P113` to completion: arm/release/re-arm, the 21-row center raster, the centroid pass, the two-footprint outer A scan, and both deferred `G54` writes. Center centroid at `MPos:-232.138,-219.475`; `G54` written `-232.136,-189.980,0.000,5649.193`; the operator then confirmed the parked pen tip perfectly centered over the center magnet. A spacing `4331.930` against the `4320 +/- 15` gate leaves only ~3 degrees of margin. See [`2026-09-24-e-18-m-08-p113-integrated-registration.md`](2026-09-24-e-18-m-08-p113-integrated-registration.md). |
