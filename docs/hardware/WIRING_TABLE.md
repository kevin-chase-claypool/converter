...
| DAQ-004 | Instructor 5 N reference sensor | bridge terminals TBD | INA101KU board | lower load-cell terminal block, mapping TBD | Reference-force analogue conditioning | Board/sensor-specific | Temporary bench harness | planned | Board photo identifies INA101KU and a lower `load cell` terminal block, but not its terminal order. Continuity-map sensor/excitation connections with power removed. |
| DAQ-005 | INA101KU board | labelled `OUT`, `GND`, `5V`, `+V`, `-V` | Pico 2 / one dual-output bench supply | `GP26`/ADC0 through 1 kOhm; `AGND`; +5 V / 0 V / −5 V rails | Reference raw voltage, bridge excitation, and conditioning power | INA101 uses ±5 V to ±20 V rails; input to Pico must remain 0-3.3 V | Temporary shielded/twisted analogue lead preferred | planned | The board `5V` terminal is not the INA101 amplifier rail. If continuity proves it is bridge excitation, branch it from the same +5 V rail that feeds `+V`; do not add a third supply. Meter unloaded and loaded `OUT` relative to `GND` before landing the ADC lead. Do not feed a negative or >3.3 V output to Pico. |
