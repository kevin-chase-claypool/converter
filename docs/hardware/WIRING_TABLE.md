...
| TH-018 | LIFT-home microswitch | Meter-verified COM/NO pair | SparkFun Pro Micro RP2350 | `GP2` and adjacent local `TOOL_GND` | Absolute `LIFT_HOME` reference at boot/recovery/service, not normal M5 | `GP2` uses its internal pullup; switch pressed pulls `GP2` LOW | Short local pair | installed; diagnostic firmware implemented; T-01G pending | Released contact was open/no-beep and pressed contact was continuous/beep before installation. The contact has no electrical polarity: either terminal may connect to `GP2`, the other to `TOOL_GND`; leave the NC terminal unused. Mount the switch fixed and its flag on the moving pen carriage. Do not connect 5 V, 6 V, or PC817C `CTRL_GND`. Firmware currently reports the input only through native USB and GP20/GP21 service UART; T-01G must pass before it controls retract. Normal M5 uses load-cell release plus a calibrated clearance pulse and must not routinely actuate this switch. |
