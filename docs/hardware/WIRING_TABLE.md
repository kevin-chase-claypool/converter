...
| DAQ-005 | INA101KU board | labelled `OUT`, `GND`, `5V`, `+V`, `-V` | Pico 2 / one dual-output bench supply | `GP26`/ADC0 through 1 kOhm; `AGND`; +5 V / 0 V / −5 V rails | Reference raw voltage, bridge excitation, and conditioning power | `+V`/`-V` are ±5 V amplifier rails; Pico input must remain 0-3.3 V | Temporary shielded/twisted analogue lead preferred | planned | TI identifies INA101KU as SOL-16: check `OUT`→pin 1, `+V`→pin 2, `-V`→pin 10, `GND`→pin 9 (`Common`) with power off. Board `5V` is not an INA101 supply pin and is likely bridge excitation; branch it from +5 V only after continuity confirms it. The 4.3 kOhm resistor likely sets G≈10.3 V/V and the 100 kOhm pot is likely offset trim. Meter `OUT` relative to `GND` before landing the ADC lead. |
