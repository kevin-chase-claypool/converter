...
| TH-018 | LIFT-home microswitch | Meter-verified COM/NO pair | SparkFun Pro Micro RP2350 | `GP2` and adjacent local `TOOL_GND` | Absolute `LIFT_HOME` reference at boot/recovery/service, not normal M5 | `GP2` uses its internal pullup; switch pressed pulls `GP2` LOW | Short local pair | installed; input and integrated service-UART verified; powered T-01G pending | Released contact was open/no-beep and pressed contact was continuous/beep before installation. The contact has no electrical polarity: either terminal may connect to `GP2`, the other to `TOOL_GND`; leave the NC terminal unused. With the motor-safe UART1 diagnostic flashed and toolhead powered from its known-good 5 V supply, COM8 at 115200 reported `lift_home=1` pressed and `0` released. Corrected integrated firmware then reported the `Theta toolhead service UART ready` line and recurring complete telemetry with `lift_home=0` released; its `T-01 actuator direction is not commissioned` fault is the intentional motor lock. Mount the switch fixed and its flag on the moving pen carriage. Do not connect 5 V, 6 V, or PC817C `CTRL_GND`. T-01G must pass before it controls retract. Normal M5 uses load-cell release plus a calibrated clearance pulse and must not routinely actuate this switch. |
