...
| PWR-013 | SparkFun Pro Micro RP2350 | `3V3` | CS1238 | `VCC` | Load-cell ADC power | 3.3 V only after received-board inspection | Short local conductor TBD | Local decoupling TBD | planned | The Pro Micro is the sole CS1238 owner; verify actual breakout supply/reference behavior in E-07C/E-07D. |
| PWR-014 | SparkFun Pro Micro RP2350 | `GND` | CS1238 | `GND` | Load-cell ADC return | 0 V | Short local conductor TBD | Same local toolhead reference | planned | Keep the ADC close to the load cell; no Pico or INA101 path remains. |
...
| MAG-003A | RP23CNC | `Aux 0` digital output | PC817C module U2 LED cathode | `AUX0` / pin 2 | Two-phase magnetic arm; end-of-print full-retract request during normal print | Installed path is active-low: `M65 P0` requests readiness, scan, or full retract, and `M64 P0` releases/disarms; `M65` -> `M64` -> `M65` enters scan | Existing routed control conductor | machine-verified, F-08 partial | On 2026-09-10, J1.4 measured 9.33 V released and 0.15 mV asserted relative to J1.5 `CTRL_GND`; the installed diagnostic observed `DISARMED → READY_ACK → WAIT_REARM → SCAN_ACTIVE → DISARMED`, followed by controller-visible PRB/G38 proof. |
...
| ID | From device | From terminal | To device | To terminal | Signal | Expected level/polarity | Wire | Status | Evidence/notes |
|---|---|---|---|---|---|---|---|---|---|
| TH-001A | RP23CNC | Spindle group `ENA` | PC817C module U1 LED cathode | `ENA` / pin 2 | M3/M5 command into isolation module | U1 LED lights only when `ENA` sinks current from controller `+5V` through `R1`. The controller `ENA` is active-high by default, so the controller sets `$16=1` (invert spindle enable): `M3` pulls `ENA` low (opto on, engage) and `M5` leaves it high (opto off, lift) | Shielded/twisted TBD | verified — F-05, 2026-09-23 | See `hardware/pc817-interface/pc817-interface.sch`; RP23U5XBB manual identifies spindle `ENA`. F-05 confirmed M3 = pen down and M5 = pen up, with pen-up fail-safe when the controller is unpowered. `ENA` meter levels at idle/M3/M5 are not yet recorded. |
| TH-001A-RT | RP23CNC | `5V`, spindle `ENA`, `Aux 0`, control `GND`, and `PROBE SIG` | Toolhead PC817C module J1.1, J1.2, J1.4, J1.5, and J1.6 respectively | Controller-side moving harness | Isolated tool command, `HOME_ARM`, and reverse `A_HOME`/probe path | Controller and toolhead ground domains remain galvanically isolated; controller `PROBE GND` is continuous with J1.5 `CTRL_GND` on this installed board | Five routed control conductors through drag chain | partial energized evidence | J1.1 `CTRL_5V`, J1.2 `ENA`, J1.4 `AUX0`, and J1.5 `CTRL_GND` were reported wired on 2026-08-23. On 2026-09-10, blue J1.6 moved from `LIMA SIG` to `PROBE SIG` after direct PRB and real GP27/U3 path tests passed. The PRB state and real-magnet A-axis G38 capture/release are verified. M3/M5 polarity is verified (`F-05`, 2026-09-23: `$16=1`, M3 down / M5 up); the macro, `F-05A`, and production-use commissioning remain open. J1.3 remains unused. |
...
| TH-006 | uxcell 300 g load cell | Red (`EXC+`) | CS1238 | `E+` | Bridge excitation positive | Manufacturer wire/function mapping | Short local conductor TBD | planned | Red `EXC+`, black `EXC-`, green `SEN+`, white `SEN-`; verify received CS1238 labels before energizing. Calibrate with precision masses through the installed pen force path. |
| TH-007 | uxcell 300 g load cell | Black (`EXC-`) | CS1238 | `E-` | Bridge excitation negative | Manufacturer wire/function mapping | Short local conductor TBD | planned | E-07D known-mass calibration required. |
| TH-008 | uxcell 300 g load cell | Green (`SEN+`) | CS1238 | `A+` | Bridge signal positive | Manufacturer wire/function mapping | Short local conductor TBD | planned | E-07D known-mass calibration required. |
| TH-009 | uxcell 300 g load cell | White (`SEN-`) | CS1238 | `A-` | Bridge signal negative | Manufacturer wire/function mapping | Short local conductor TBD | planned | E-07D known-mass calibration required. |
| TH-010 | CS1238 | `DT`/`DRDY` | SparkFun Pro Micro RP2350 | `GP0` | Load-cell data | 3.3 V logic | Short local conductor | planned | Pro Micro is the only ADC owner. |
| TH-011 | SparkFun Pro Micro RP2350 | `GP1` | CS1238 | `SCK` | Load-cell clock | 3.3 V logic | Short local conductor | planned | Do not parallel a second controller. |
