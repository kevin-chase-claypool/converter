...
| PWR-003 | HCDC HD064RT | `OUT6 +` | X TB6600 | `VCC`/`DC+` TBD | X stepper power | 12 VDC nominal; E-stop does not remove this power | 20 AWG, owner-reported | `OUT6` branch fuse, 3 A selected; confirm fitted marking | planned — wired, unverified | Owner reports the positive conductor is physically landed from `OUT6`; owner selects a 3 A branch fuse. Confirm exact driver terminal, polarity, fitted fuse marking, and continuity before power. Do not bypass the HD064RT branch fuse. |
| PWR-004 | HCDC HD064RT | `OUT6 -` | X TB6600 | `GND`/`DC-` TBD | X stepper return | 0 VDC | 20 AWG, owner-reported | Paired with PWR-003 | planned — wired, unverified | Owner reports the return conductor is physically landed; confirm exact driver terminal and continuity with power removed. |
| PWR-005 | HCDC HD064RT | `OUT7 +` | Y TB6600 | `VCC`/`DC+` TBD | Y stepper power | 12 VDC nominal; E-stop does not remove this power | 20 AWG, owner-reported | `OUT7` branch fuse, 3 A selected; confirm fitted marking | planned — wired, unverified | Owner reports the positive conductor is physically landed from `OUT7`; owner selects a 3 A branch fuse. Confirm exact driver terminal, polarity, fitted fuse marking, and continuity before power. Do not bypass the HD064RT branch fuse. |
| PWR-006 | HCDC HD064RT | `OUT7 -` | Y TB6600 | `GND`/`DC-` TBD | Y stepper return | 0 VDC | 20 AWG, owner-reported | Paired with PWR-005 | planned — wired, unverified | Owner reports the return conductor is physically landed; confirm exact driver terminal and continuity with power removed. |
| PWR-007 | HCDC HD064RT | `OUT8 +` | A TB6600 | `VCC`/`DC+` TBD | A stepper power | 12 VDC nominal; E-stop does not remove this power | 20 AWG, owner-reported | `OUT8` branch fuse, 3 A selected; confirm fitted marking | planned — wired, unverified | Owner reports the positive conductor is physically landed from `OUT8`; owner selects a 3 A branch fuse. Confirm exact driver terminal, polarity, fitted fuse marking, and continuity before power. Do not bypass the HD064RT branch fuse. |
| PWR-008 | HCDC HD064RT | `OUT8 -` | A TB6600 | `GND`/`DC-` TBD | A stepper return | 0 VDC | 20 AWG, owner-reported | Paired with PWR-007 | planned — wired, unverified | Owner reports the return conductor is physically landed; confirm exact driver terminal and continuity with power removed. |
| PWR-009A | HCDC HD064RT | `OUT4 +` | Pololu D36V50F6 | `VIN` | Toolhead 6 V regulator input | 12 VDC nominal; E-stop does not remove this power; regulator accepts 6.5-50 V input | TBD | `OUT4` branch fuse, 3 A intended; confirm fitted marking | verified | E-14 passed 2026-09-08; owner reported correct polarity and constant 6.05 V regulator output. Branch-fuse marking/current characterization remains E-15. |
| PWR-009B | HCDC HD064RT | `OUT4 -` | Pololu D36V50F6 | `GND` | Toolhead 6 V regulator return | 0 VDC | TBD | Paired with PWR-009A | verified | E-14 passed; return path supports the 6.05 V output measurement. |
| PWR-009 | Pololu D36V50F6 | `VOUT` via 2-pin toolhead JST | DRV8833 | `VCC`/`VM` TBD | Actuator motor power | Fixed 6.0 V output | 20 AWG red/black twisted pair TBD | Output fuse/capacitance TBD | verified | E-14 passed 2026-09-08 with constant 6.05 V reported. E-15 remains the loaded ripple/current/temperature characterization. |
...
The supplied TB6600 schematic defines the common-cathode pattern: each RP23CNC
axis `G` is jumpered to its TB6600 `PUL-`, `DIR-`, and `ENA-`; `Stp`, `Dir`, and
`En` go to `PUL+`, `DIR+`, and `ENA+`. E-03 installed-driver verification passed
on 2026-09-05 for X, Y, and A. Compare every terminal with received hardware
...
for all of these conductors. On 2026-09-05, the powered E-03 test passed on all
three installed drivers: `ENA+` was active-low (about 5 V idle and 0 V moving),
`DIR+` changed state for opposite motion, and `PUL+` produced approximately
5 V step pulses.

| ID | From device | From terminal | To device | To terminal | Signal | Expected level/polarity | Wire | Status | Evidence/notes |
|---|---|---|---|---|---|---|---|---|---|
| MOT-001 | RP23CNC | X `Stp` | X TB6600 | `PUL+` | X step pulse | 5 V pulse activity during commanded motion | 24 AWG blue | bench-verified | F-03 and E-03 passed; installed-driver signal response recorded 2026-09-05. |
| MOT-002 | RP23CNC | X `Dir` | X TB6600 | `DIR+` | X direction | 0 V positive, 5 V negative direction | 24 AWG white | bench-verified | F-03 and E-03 passed; installed-driver signal response recorded 2026-09-05. |
| MOT-003 | RP23CNC | X `En` | X TB6600 | `ENA+` | X enable | Active-low: 5 V idle, 0 V moving | 24 AWG yellow | bench-verified | F-03 and E-03 passed; installed-driver signal response recorded 2026-09-05. |
| MOT-004 | RP23CNC | Y `Stp` / `Dir` / `En` | Y TB6600 | `PUL+` / `DIR+` / `ENA+` | Y step, direction, enable | Same F-03 logic as X | 24 AWG blue / white / yellow | bench-verified | Owner reports the installed-driver response matches X/A; E-03 recorded 2026-09-05. |
| MOT-005 | RP23CNC | A `Stp` / `Dir` / `En` | A TB6600 | `PUL+` / `DIR+` / `ENA+` | A step, direction, enable | Same F-03 logic as X | 24 AWG blue / white / yellow | bench-verified | A-axis installed-driver response passed 2026-09-05; X/Y reported matching. |
| MOT-006 | RP23CNC | X `G` | X TB6600 | `PUL-`, `DIR-`, `ENA-` via common block | X signal return | Axis-local common reference | 24 AWG black; three common-block jumpers | bench-verified | Common return continuity and E-03 signal response passed; do not use isolated-input ground. |
| MOT-007 | RP23CNC | Y `G` | Y TB6600 | `PUL-`, `DIR-`, `ENA-` via common block | Y signal return | Axis-local common reference | 24 AWG black; three common-block jumpers | bench-verified | Common return continuity and E-03 signal response passed; do not use isolated-input ground. |
| MOT-008 | RP23CNC | A `G` | A TB6600 | `PUL-`, `DIR-`, `ENA-` via common block | A signal return | Axis-local common reference | 24 AWG black; three common-block jumpers | bench-verified | Common return continuity and E-03 signal response passed; do not use isolated-input ground. |
...
| SAF-004 | SW1 mxuteuk `HB2-BS544` E-stop, NC-A | Upper `1`/`NC`/`2` contact block in supplied rear-switch photo; use its terminal `1` and terminal `2` | RP23CNC | `ESTOP`: `SIG` and `GND` (rightmost pair in the Grbl Control Inputs group, after `FD HOLD`) | Immediate grblHAL Halt | NC released/healthy, open when pressed; NC input configuration `$14=6` | Low-current protected control wire TBD | verified | Run one wire from either NC-A terminal to `SIG` and the other to `GND`; the two assignments are interchangeable. Never mix terminals between NC-A and NC-B. On 2026-09-08, power-off meter test gave continuity/beep released and open/no beep pressed; `$14=6` plus press produced ioSender `ALARM:10`; twist-release → Reset → Unlock returned it to `IDLE` with no motion reported. This controller-signal stop leaves motor/tool 12 V powered; see `ESTOP_TOPOLOGY.md`. |
| SAF-005 | SW1 mxuteuk `HB2-BS544` E-stop, NC-B | Lower independent `1`/`NC`/`2` contact block in supplied rear-switch photo | None | Both terminals individually insulated | Unused | No electrical connection | None | planned | K1 is not part of this project. Do not connect NC-B to power, ground, or NC-A. |
...
| SAF-007 | HCDC `HD064RT` | `OUT6` / fused pair | X TB6600 | 12 V input | X motor power | Remains powered when SW1 is pressed | 20 AWG, fuse 3 A selected | planned | Owner reports branch wired; confirm fitted marking/current before power. |
| SAF-008 | HCDC `HD064RT` | `OUT7` / fused pair | Y TB6600 | 12 V input | Y motor power | Remains powered when SW1 is pressed | 20 AWG, fuse 3 A selected | planned | Owner reports branch wired; confirm fitted marking/current before power. |
| SAF-009 | HCDC `HD064RT` | `OUT8` / fused pair | A TB6600 | 12 V input | A motor power | Remains powered when SW1 is pressed | 20 AWG, fuse 3 A selected | planned | Owner reports branch wired; confirm fitted marking/current before power. |
...
| TH-001A-RT | RP23CNC | `5V`, spindle `ENA`, `Aux 0`, control `GND`, and `PROBE SIG` | Toolhead PC817C module J1.1, J1.2, J1.4, J1.5, and J1.6 respectively | Controller-side moving harness | Isolated tool command, `HOME_ARM`, and reverse `A_HOME`/probe path | Controller and toolhead ground domains remain galvanically isolated; controller `PROBE GND` is continuous with J1.5 `CTRL_GND` on this installed board | Five routed control conductors through drag chain | partial energized evidence | J1.1 `CTRL_5V`, J1.2 `ENA`, J1.4 `AUX0`, and J1.5 `CTRL_GND` were reported wired on 2026-08-23. On 2026-09-10, blue J1.6 moved from `LIMA SIG` to `PROBE SIG` after direct PRB and real GP27/U3 path tests passed. The PRB state and real-magnet A-axis G38 capture/release are verified; M3/M5, macro, and production-use commissioning remain open. J1.3 remains unused. |
...
| TH-003A | SparkFun Pro Micro RP2350 | `GP6` | DRV8833 | `EEP` / low-true sleep input | Driver enable/sleep | Drive HIGH to enable, LOW to sleep | Short local conductor | continuity-checked; function re-test pending | The owner confirmed the installed module's pin-1 silkscreen is `SLEEP`/`EEP`. Keep the installed GP6→`EEP` wire; firmware must drive it. |
| TH-003B | DRV8833 | `ULT` / low-true protection-fault output | SparkFun Pro Micro RP2350 | `GP7` | Driver fault input | Open-drain-style active-low fault; firmware uses `INPUT_PULLUP` | Short local conductor | continuity-checked; function re-test pending | The owner confirmed the installed module's pin-6 silkscreen is `FAULT`/`ULT`. Keep the installed `ULT`→GP7 wire; firmware must read it. |
| TH-003C | SparkFun Pro Micro RP2350 | `GP4`, `GP5`, `GP6`, `GP7` | DRV8833 | logic connector | One 1×4 direct-header harness | Consecutive left-side GPIO run; GND is supplied by the separate DRV8833 power connector | Short local harness | continuity-checked | Installed harness is GP4→IN1, GP5→IN2, GP6→`EEP`/sleep, and `ULT`/fault→GP7. The earlier opposite `EEP`/`ULT` role record is superseded by the installed-board label confirmation. |
...
| TH-004 | DRV8833 | `OUT1` | N20 actuator | Motor lead 1 | Actuator drive | First E-05 pulse retracts/lifts the pen | Existing temporary motor pair; final 22 AWG red/black twisted pair pending | Local bulk capacitance at driver | repair required | Isolated 2026-09-13 output test passed OUT1 high/OUT2 low, but installed 1000 RPM N20 did not respond. With power removed, inspect and re-terminate/resolder this motor-side connection; do not alter GP4–GP7. |
| TH-005 | DRV8833 | `OUT2` | N20 actuator | Motor lead 2 | Actuator drive | Second E-05 pulse moves the pen down | Existing temporary motor pair; final 22 AWG red/black twisted pair pending | Local bulk capacitance at driver | repair required | Isolated 2026-09-13 output test passed OUT1 low/OUT2 high, but installed 1000 RPM N20 did not respond. With power removed, inspect and re-terminate/resolder this motor-side connection; do not alter GP4–GP7. |
...
| TH-018 | LIFT-home microswitch | Meter-verified COM/NO pair | SparkFun Pro Micro RP2350 | `GP2` and adjacent local `TOOL_GND` | Absolute `LIFT_HOME` reference at boot/recovery/service, not normal M5 | `GP2` uses its internal pullup; switch pressed pulls `GP2` LOW | Short local pair | installed; end-to-end input/UART verified; powered T-01G pending | Released contact was open/no-beep and pressed contact was continuous/beep before installation. The contact has no electrical polarity: either terminal may connect to `GP2`, the other to `TOOL_GND`; leave the NC terminal unused. With the motor-safe UART1 diagnostic flashed and toolhead powered from its known-good 5 V supply, COM8 at 115200 reported `lift_home=1` pressed and `0` released. Corrected integrated firmware then reported the `Theta toolhead service UART ready` line and recurring complete telemetry; manual switch operation yielded the expected `lift_home=0` released → `1` pressed → `0` released. Its `T-01 actuator direction is not commissioned` fault is the intentional motor lock. Mount the switch fixed and its flag on the moving pen carriage. Do not connect 5 V, 6 V, or PC817C `CTRL_GND`. Powered T-01G must pass before it controls retract. Normal M5 uses load-cell release plus a calibrated clearance pulse and must not routinely actuate this switch. |
...
| DAQ-004 | Instructor 5 N reference sensor | integral four-pad load-cell footprint | INA101KU board | internal board connection | Reference-force analogue conditioning | Board/sensor-specific | No user-installed sensor harness | planned | Supplied PCB artwork shows the load cell is integrated with this board assembly. The rear green five-position terminal block is the only user wiring interface; its individual bridge-pad map is not needed for the test. |
| DAQ-005 | INA101KU board | labelled `OUT`, `GND`, `5V`, `+V`, `-V` | Pico 2 / one dual-output bench supply | `GP26`/ADC0 through 1 kOhm; `AGND`; +5 V / 0 V / −5 V rails | Reference raw voltage, bridge excitation, and conditioning power | `+V`/`-V` are ±5 V amplifier rails; Pico input must remain 0-3.3 V | Temporary shielded/twisted analogue lead preferred | planned | Owner confirms board `5V` is the original Arduino 5 V bridge-excitation input. Branch the bench +5 V rail to both `+V` and `5V`; it is not a third supply or an INA101 rail. TI identifies INA101KU as SOL-16: check `OUT`→pin 1, `+V`→pin 2, `-V`→pin 10, `GND`→pin 9 (`Common`) with power off. The 4.3 kOhm resistor likely sets G≈10.3 V/V and the 100 kOhm pot is likely offset trim. Meter `OUT` relative to `GND` before landing the ADC lead. |
| DAQ-006 | Pico 2 | micro-USB | Host PC | USB | DAQ power and timestamped serial data | USB CDC serial | USB data cable | planned | Native Pico SDK `pico2_dual_sensor_daq` emits raw `SAMPLE` and marker `EVENT` records. PC logs raw records and metadata; Windows receive time is not the sensor time base. |
| DAQ-007 | Pico 2 | `GP15`, `GND` | latching SPST ON/OFF switch | switch contacts | Optional physical DAQ run control | `INPUT_PULLUP`; ON closes GP15 to GND/LOW, OFF opens/HIGH | Temporary two-wire lead | planned | ON starts a DAQ run and OFF stops it. This switch is DAQ control only; it does not control actuator power or replace the machine E-stop/main-power cutoff. |
| DAQ-008 | Existing toolhead 6 V input | Existing 2-pin JST / established local rail | DRV8833 and S7V8F5 | Motor rail; regulator `VIN` | Controlled downward/upward pen actuation during loaded calibration | 6.0 V nominal, established path | Existing toolhead harness | planned for supervised DAQ test | DRV8833 receives 6 V; S7V8F5 produces regulated 5 V for the Pro Micro. Do not feed 6 V directly to the Pro Micro. E-stop does not by itself remove this rail; verify a manual power-cutoff path before loading. |
| DAQ-009 | DSD TECH SH-U09C2 service adapter | `RXD`, `TXD`, `GND` | SparkFun Pro Micro RP2350 | `GP20`/UART1 TX, `GP21`/UART1 RX, `TOOL_GND` | Pro Micro actuator commands for loaded calibration | 3.3 V UART, 115200 baud | Existing temporary service harness | bench-verified interface; test procedure planned | Cross TX/RX; adapter VCC remains disconnected. This command link controls motion only; Pico timestamps remain the force-data time base. See TH-016/TH-017. |
| DAQ-010 | SparkFun Pro Micro RP2350 | temporary `GP1`, `TOOL_GND` | Pico 2 | `GP14`, `GND` | `MOTION_ACTIVE` timing marker | GP1 is 3.3 V output; Pico records rising/falling edges; marker idle LOW | Separate temporary two-wire pair | planned | GP1 is disconnected from CS1238 clock duty only during this fixture. Connect the signal reference only to `TOOL_GND`, never PC817C `CTRL_GND`. No 5 V level is permitted. |
...
|---|---:|---|---|---|
| 2026-09-10 | 4.7 | Moved the existing blue GP27/U3 `A_HOME` conductor from `LIMA SIG` to `PROBE SIG` after direct and real-magnet motor-inert PRB/G38 entry/release checks passed; recorded installed active-low Aux0 command mapping and `$6=1`. | Codex | `RPSW-20260910-002`; `2026-09-10-e-18-motor-inert-p100-handshake.md` |
...
| 2026-09-16 | 4.9 | Added a planned temporary Pro Micro GP1 -> Pico GP14 `MOTION_ACTIVE` marker and its `TOOL_GND` signal reference for Pico-clocked actuator-edge timestamps. | Codex | `HW-20260915-001`; `PICO2_DUAL_SENSOR_DAQ.md` |
| 2026-09-16 | 5.0 | Corrected the temporary INA101 fixture to show one series-linked dual-output bench supply: if continuity verifies the `5V` bridge-excitation terminal, it branches from the +5 V rail rather than a separate supply. Also removed diagram label collisions. | Codex | `HW-20260915-001`; `PICO2_DUAL_SENSOR_DAQ.md` |
| 2026-09-16 | 5.1 | Replaced the temporary GP15 momentary START/STOP input with a latching SPST DAQ ON/OFF switch: ON grounds GP15/LOW and OFF releases it/HIGH. | Codex | `HW-20260915-001`; `PICO2_DUAL_SENSOR_DAQ.md` |
| 2026-09-19 | 5.4 | Added native Pico 2 fixture firmware for the planned GP2/GP3 CS1238, GP26 ADC0, GP14 marker, GP15 switch, and USB CDC stream. No physical wiring is verified or changed by this source addition. | Codex | `pico2_dual_sensor_daq`; E-09C |
| 2026-09-18 | 5.2 | Reconciled the instructor INA101KU PCB artwork with TI documentation: the reference load cell is integral to the board; `+V`/`-V` are amplifier rails; `5V` remains a continuity-gated likely bridge-excitation terminal; 4.3 kOhm is the likely gain resistor and 100 kOhm the likely offset trim. | Codex | `2026-09-18-ina101ku-instructor-pcb-datasheet-reconciliation.md` |
| 2026-09-18 | 5.3 | Owner confirmed the instructor board's `5V` terminal was powered by Arduino 5 V in its original setup. It is now the confirmed reference-bridge excitation input and branches from the fixture's +5 V rail alongside INA `+V`; no third supply is required. | Codex | Owner report; `PICO2_DUAL_SENSOR_DAQ.md` |
...
| 2026-09-13 | 4.8 | Corrected the confirmed installed DRV8833 module-label mapping without solder rework: GP6 drives `EEP`/sleep and GP7 reads `ULT`/fault. The prior opposite-role record is superseded; reflash and re-run E-14C/T-01 before relying on driver behavior. | Codex | Owner-confirmed installed-board photo; `HW-20260913-001`; E-14C function re-test required |
| 2026-08-13 | 2.7 | Historical record superseded 2026-09-13: it incorrectly assigned the installed module's `ULT`/`EEP` roles. | Codex | See revision 2.8 and `HW-20260913-001`. |
