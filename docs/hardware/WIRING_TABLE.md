...
| DAQ-005 | INA101KU board | labelled `OUT`, `GND`, `+V`, `-V` | Pico 2 / dual bench supply | `GP26`/ADC0 through 1 kOhm; `AGND`; verified ± rails | Reference raw voltage and conditioning power | INA101 uses ±5 V to ±20 V rails; input to Pico must remain 0-3.3 V | Temporary shielded/twisted analogue lead preferred | planned | The board's `5V` terminal is not the INA101 supply rail. Meter unloaded and loaded `OUT` relative to `GND` before landing the ADC lead. Do not feed a negative or >3.3 V output to Pico. |
