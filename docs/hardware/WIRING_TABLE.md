...
| SAF-004 | SW1 mxuteuk `HB2-BS544` E-stop, NC-A | Upper `1`/`NC`/`2` contact block in supplied rear-switch photo; use its terminal `1` and terminal `2` | RP23CNC | `ESTOP`: `SIG` and `GND` (rightmost pair in the Grbl Control Inputs group, after `FD HOLD`) | Immediate grblHAL Halt | NC released/healthy, open when pressed; use NC input configuration (planned `$14=6`, verify live) | Low-current protected control wire TBD | partial — owner reports wired; live verification pending | Run one wire from either NC-A terminal to `SIG` and the other to `GND`; the two assignments are interchangeable. Never mix terminals between NC-A and NC-B. This controller-signal stop leaves motor/tool 12 V powered. Terminal identified from the RP23CNC user manual "Key Features" board diagram (p.6, board rev RP23U5XBB V1.0) and confirmed against the owner's installed V1.01 board photo. Owner reported the connection landed on 2026-09-08; E-19 is still required before marking verified. See `ESTOP_TOPOLOGY.md`. |
