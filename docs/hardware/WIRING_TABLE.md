...
| SAF-004 | SW1 mxuteuk `HB2-BS544` E-stop, NC-A | Contact pair A | RP23CNC | `ESTOP`: `SIG` and `GND` (rightmost pair in the Grbl Control Inputs group, after `FD HOLD`) | Immediate grblHAL Halt | NC released/healthy, open when pressed; use NC input configuration (planned `$14=6`, verify live) | Low-current protected control wire TBD | planned — terminal identified, not yet landed | Terminal identified from the RP23CNC user manual "Key Features" board diagram (p.6, board rev RP23U5XBB V1.0) and confirmed against the owner's installed V1.01 board photo, which shows matching `... DOOR CY/ST FD HOLD ESTOP` silkscreen with `SIG`/`GND` pairs. No wire has been landed and E-19 has not been performed; see `docs/report/lab-notes/2026-09-08-rp23cnc-estop-terminal-identification.md`. |
