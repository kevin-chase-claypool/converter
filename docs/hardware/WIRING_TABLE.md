...
| DAQ-006 | Pico 2 | micro-USB | Host PC | USB | DAQ power and timestamped serial data | USB CDC serial | USB data cable | planned | Arduino-Pico `force_calibration_test/pico2_daq` emits raw `SAMPLE` records. PC logs raw records and metadata; Windows receive time is not the sensor time base. |
| DAQ-007 | SparkFun Pro Micro RP2350 | temporary `GP0`, `TOOL_GND` | Pico 2 | `GP15`, `GND` | DAQ capture gate | 3.3 V only; Pro Micro HIGH/reset stops Pico, LOW starts it | Temporary two-wire lead | planned | GP0 is disconnected from CS1238 data duty only during this fixture. The dedicated test sketch waits 100 ms after asserting LOW before motion, then returns HIGH after settle. Never use PC817C `CTRL_GND`; no 5 V level is permitted. |
