...
| SAF-004 | SW1 mxuteuk `HB2-BS544` E-stop, NC-A | Upper `1`/`NC`/`2` contact block in supplied rear-switch photo; use its terminal `1` and terminal `2` | RP23CNC | `ESTOP`: `SIG` and `GND` (rightmost pair in the Grbl Control Inputs group, after `FD HOLD`) | Immediate grblHAL Halt | NC released/healthy, open when pressed; NC input configuration `$14=6` | Low-current protected control wire TBD | partial — live E-stop cycle passed; meter check pending | Run one wire from either NC-A terminal to `SIG` and the other to `GND`; the two assignments are interchangeable. Never mix terminals between NC-A and NC-B. With `$14=6`, pressing SW1 produced ioSender `ALARM:10`; after twist-release, Reset, and Unlock it returned to `IDLE` without reported motion (2026-09-08). This controller-signal stop leaves motor/tool 12 V powered. Unpowered NC-A/NC-B continuity evidence remains required; see `ESTOP_TOPOLOGY.md`. |
