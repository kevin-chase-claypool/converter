...
| 3 | Amazon ASIN B0FQ5GBNZ1 | Listed as TB6600, 9-42 V, up to 4 A, step/direction, three-pack. Reported factory setting: only switches 2 and 4 ON; 1, 3, 5, and 6 OFF. On the received printed table this corresponds to 4 microsteps (800 pulses/rev) and 2.0 A. | selected | Confirm every received unit's switch numbering/table. Before motor power, change the current setting to the 1.5 A motor-rated row if the printed table agrees: SW4 ON, SW5 OFF, SW6 ON. Verify input polarity and thermal behavior. |
