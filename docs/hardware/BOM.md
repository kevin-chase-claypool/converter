...
| 2 | CS1238 load-cell ADC breakouts, HX711-form-factor, exact seller/revision pending inspection | One is the production replacement candidate and the second is available for the Pico 2 dual-sensor calibration fixture if the instructor reference sensor proves electrically compatible. The boards are expected to expose `VCC/GND/DT/SCK` and `E+/E-/A+/A-`; CS1238 #1 retains the four-wire 300 g toolhead load cell and ultimately returns to the Pro Micro's GP0→`DT`/`DRDY-DOUT` and GP1→`SCK` path. | received/owner-reported; uninspected | Confirm each received board's silkscreen, 3.3 V range, reference circuit, header order, mounting-hole alignment, and channel-A terminal mapping. Do not connect two ADCs to one bridge simultaneously. Run E-07C through E-09C before enabling force control. |
