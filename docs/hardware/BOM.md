...
| 1 | HCDC `HD064RT` DIN-rail 8-channel DC fuse distribution module | Installed module; 5-32 V DC, eight pluggable fused outputs, 20 A aggregate specification, and factory 3 A fuses. Allocated from the protected 12 V bus to X, Y, A, and D36V50F6; outputs 5-8 reserved. | received | Verify DC-input/output polarity, continuity, actual fuse type, and branch current before energizing loads |
...
| 3 | Amazon ASIN B0FQ5GBNZ1 | Listed as TB6600, 9-42 V, up to 4 A, step/direction, three-pack. Received label photograph confirms factory state: only switches 2 and 4 ON; 1, 3, 5, and 6 OFF. The printed table maps this to 8 microsteps (1600 pulses/rev) and 2.0 A. The project baseline for all three 1.8 degree motors is 8 microsteps (`SW1 OFF`, `SW2 ON`, `SW3 OFF`); for the 12:1 A drive this gives 19,200 pulses/bed revolution and 0.01875 bed degree/pulse. | selected | Confirm every received unit's switch numbering/table. Before motor power, change the current setting to the 1.5 A motor-rated row if the printed table agrees: SW4 ON, SW5 ON, SW6 OFF. Verify input polarity and thermal behavior. |
| TBD length | KWANGIL 20 AWG 4C AMESB shielded cable, Amazon ASIN B0GVBF51Q7 | Purchased 4-conductor UL2464-style shielded cable for NEMA17 phase wiring; listing identifies flexible stranded tinned-copper conductors, overall shield with drain, and tinned-copper braid (`OS+Drain+TC BRD`) for CNC/stepper use. The Y axis instead uses its already-installed shielded 4C cable, Amazon ASIN B0DL9QCH1B, with black/green/red/white conductors. | purchased | Confirm received gauge/markings, flexibility in the drag chain, conductor colors, drain-wire continuity to shield, and fit through strain reliefs/connectors. Y cable splice is documented in the master wiring table. |
