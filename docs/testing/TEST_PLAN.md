...
| E-09F | Guarded CS1238 force-hold and pen-clear qualification | `s` reaches/holds the precision-weight 40–60 g raw band for five seconds without fault, excess pulse budget, timeout, or implausible scale motion; separate `c` reaches the 3 g clear band then proves the 100 ms candidate air-gap motion. | Partial — hold passed | Two `t`/`a`/`s` runs reported stable 40.0 g and 40.4 g scale values, passing the requested practical 40–60 g band check. E-09F intentionally stops correction on entering the band, so the lower-edge result is expected and does not claim a centered 50 g hold. The `a`/`c` 100 ms air-gap clearance test remains. Runtime is the existing GP20/GP21 3.3 V service adapter with VCC disconnected. This one-cycle bench test does not enable M3/M5, GP27, or production drawing; T-01H retains its repeated-clear-cycle acceptance requirement. |
