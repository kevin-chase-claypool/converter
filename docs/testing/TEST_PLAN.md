...
| E-09C | Calibrate installed CS1238/load-cell with known precision masses | Pro Micro raw CS1238 captures at 0–70 g from the repeatable downward motor-mount weight fixture establish a raw-to-grams relationship, stationary noise, loading/unloading hysteresis, residual bounds, and report-ready figures. The app separately labels the approximate opposite-direction pen-force projection. | Planned | Keep the 6 V actuator rail disconnected. Run `e07d_cs1238_known_mass_calibration` and its one-COM-port Windows app. Capture 0, 5, 10, …, 70 g through at least three loading and three unloading passes. Retain every raw CSV and examine time traces, the measured downward-weight fit, the estimated upward pen-force projection, and residual graph before selecting an initial 40–60 g raw band. Confirm raw direction once with the installed pen before any control gate. Pico 2, INA101KU, and the 5 N reference fixture are not used. |
