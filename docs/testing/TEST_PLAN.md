...
**T-01A/T-01G position result (2026-09-09):** spring-seat separation measured
`7.23 mm` at first `lift_home=1` assertion and `7.97 mm` at first release, a
`0.74 mm` observed switch window. The asserted position is `5.28 mm` above the
`1.95 mm` housing endpoint. This is not a coil-bind or approved operating
margin until free-spring `L_solid` and the actual mechanical backstop relation
are measured.
