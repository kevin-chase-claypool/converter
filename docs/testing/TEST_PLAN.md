...
| E-09F | Guarded CS1238 force-hold and pen-clear qualification | `s` reaches/holds the precision-weight 40–60 g raw band for five seconds without fault, excess pulse budget, timeout, or implausible scale motion; separate `c` reaches the 3 g clear band then proves the 100 ms candidate air-gap motion. | Partial — serial-confirmed one-cycle hold/clear passed | Two `t`/`a`/`s` runs reported stable 40.0 g and 40.4 g scale values, passing the requested practical 40–60 g band check. E-09F intentionally stops correction on entering the band, so the lower-edge result is expected and does not claim a centered 50 g hold. In the latest clear, release reached `tare_delta=1020` (within the ±15,116 raw / 3 g band), then the 100 ms pulse visibly produced about 1.75 mm of clearance without reaching GP2 LIFT_HOME. It waited 500 ms, recorded `AIR_GAP_SETTLED,tare_delta=93479`, and printed `CLEAR_COMPLETE` with the driver asleep. The delayed post-gap value is strain telemetry only; clear was proven before the gap pulse. Runtime is the existing GP20/GP21 3.3 V service adapter with VCC disconnected. This one-cycle bench test does not enable M3/M5, GP27, or production drawing; T-01H retains its repeated-clear-cycle acceptance requirement. |
