...
| E-14C | Inspect and function-check ACEIRMC DRV8833 control labels | Retain GP6→`EEP` (sleep input) and `ULT` (protection/fault output)→GP7; firmware drives `EEP` HIGH to enable and reads `ULT` with `INPUT_PULLUP`; J2 state is recorded | Partial | The former pass is superseded by the owner-confirmed installed-board labels: `EEP` is SLEEP and `ULT` is FAULT. Firmware mapping was corrected 2026-09-13; function re-test with the actuator is required before T-01 proceeds. |
