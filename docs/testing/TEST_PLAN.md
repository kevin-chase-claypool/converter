...
| E-06 | Measure current-limited actuator stall current | Below verified DRV8833 safe limit | Partial | Owner clarifies that the observed `0.18 A` at the retract endpoint is the N20 stall current, and reports that the motor holds the selected spring preload. The current-limited supply setup, stall duration, peak-capture method, rail behavior, driver fault state, and temperature were not recorded, so the formal electrical/thermal safety pass remains open. |
...
formal E-06 or T-01C/T-01D pass because the supply voltage/current limit, stall
duration, peak-capture method, PWM, spring length, end-stop condition, force,
rail behavior, and temperatures were not recorded. Do not use a fully
compressed spring as a normal operating point. See
