...
| E-18 | Verify Pro Micro RP2350/TMAG5273 magnetic interface | Qwiic readings are stable; service diagnostics are readable; the `A_HOME` output driver is electrically compatible with the selected RP23CNC input before connection | Partial | The 2026-09-10 motor-inert diagnostic passed the installed active-low Aux0/U2/GP28 first-arm/release/re-arm sequence and local TMAG `detected=0→1→0` scan state; J1.4 measured 9.33 V released and 0.15 mV asserted relative to `CTRL_GND`. The GP27/U3 return sank J1.6 `A_HOME` to 0 V in `READY_ACK`, but ioSender exposed no controller-side `LIMA` state, so input interpretation, release voltage, and the `PRB`/G38 stages remain unverified. See `docs/report/lab-notes/2026-09-10-e-18-motor-inert-p100-handshake.md`. |
