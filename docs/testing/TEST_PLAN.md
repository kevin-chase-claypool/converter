...
| E-09C | Calibrate installed CS1238/load-cell with known precision masses | Partial — 2026-09-22 one loading and one unloading 0–90 g run (19 captures) produced `R²=0.999231`, `0.705 g` RMS residual, and a 4,988.35 raw/g downward-weight scale. A 2.5 g pen cap was present at every point, so use the app's recorded +2.5 g fixture-label correction before relying on the saved zero. The opposite-direction projection stages 35/50/70 g pen-force candidates, but does not prove the installed upward pen path. | Partial | Actuator 6 V remained disconnected. Raw evidence is retained in `e07d_cs1238_known_mass_calibration/pc_logger/results/run_2026-09-22_08-29-03/` and summarized in the dated lab note. Open that run in the application, add the 2.5 g fixture mass, and fit again; raw traces remain unchanged. Then perform the installed-pen kitchen-scale check near 50 g to confirm raw direction before any control gate. Complete actuator direction/response and M5-clearance tests. Pico 2, INA101KU, and the 5 N reference fixture are not used. |
