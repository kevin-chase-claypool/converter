...
| F-05A | Validate commissioned GP27/PRB pen-transition acknowledgement | `P115 Q0` accepts a verified initial clear indication; after each M3/M5 command `P115 Q1` observes GP27/PRB inactive then active before the next motion block. A held high, held low, or delayed signal reports `error[39]`; P115 itself issues no motion, M3/M5, Aux0, or actuator command. | Planned | First prove the macro against an isolated PRB dry-contact fixture with all motors/tool actuation inhibited, record `$6`, timing, and error behavior. Then repeat only after F-05, E-09C, T-01H, and the relevant controller gates pass. `GP27_NORMAL_STATUS_ENABLED` and the converter handshake checkbox remain false until this evidence is accepted. |
