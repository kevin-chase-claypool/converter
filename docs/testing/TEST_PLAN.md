...
| **T-01C — static motor hold against preload** | After E-06 and E-15, command the motor to the worst intended opposing-spring position, not a hard stop. At nominal 6 V, record actuator position, pen force, rail voltage, motor/driver current, and motor/driver/regulator temperature at the start and after a defined hold dwell. Repeat once with the driver disabled only if the intended LIFT/HOLD design allows it, to determine back-drive or creep. The prior 0.18 A result was measured while pressing the LIFT_HOME switch at the travel endpoint and is not a T-01C preload-hold measurement. | T-01C remains open until a known safe compression is held for the defined dwell with position/force/rail/current/temperature and drift recorded. The endpoint-stall result may remain as separate bounded E-06 evidence, but it does not prove normal preload capability. |
