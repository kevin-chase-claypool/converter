...
| E-09C | Calibrate installed CS1238/load-cell with known precision masses | Partial — the cap-free 2026-09-22 loading/unloading 0–90 g repeat (20 captures) produced `R²=0.999978`, `0.132 g` RMS residual, and a 5,038.77 raw/g downward-weight scale. It supersedes the earlier cap-included run for staged candidates. E-09E confirms the opposite raw direction, but its initial 40.7/62.5/2.2 g observations are not monotonic enough to fit; a 10 ms up pulse alone reduced the scale from 35.5 g to 2.2 g. | Partial | Actuator 6 V remained disconnected for the known-mass fit. Accepted raw evidence is retained in `e07d_cs1238_known_mass_calibration/pc_logger/results/run_2026-09-22_09-10-46/` and summarized in the cap-free-repeat lab note. Use E-09E for installed-pen kitchen-scale checks: its runtime link is the existing GP20/GP21 3.3 V USB-to-TTL adapter with adapter VCC disconnected, avoiding a second 5 V USB-C source. Arduino IDE Serial Monitor at 115200 is the preferred manual interface: `t`, `a`, `d`, `r`, `u`, `x`, `[`/`]` and `?` operate the bounded check. It permits only operator-supervised individual 5–100 ms N20 pulses and does not auto-seek scale force. Start at 5 ms. Repeat settled points with each pulse duration/count recorded before revising any control candidate. The staged 100 ms M5 clearance pulse remains unvalidated until T-01H measures its actual gap and succeeds in 30 cycles. Confirm direction before any control gate, then complete actuator direction/response and M5-clearance tests. Pico 2, INA101KU, and the 5 N reference fixture are not used. |
