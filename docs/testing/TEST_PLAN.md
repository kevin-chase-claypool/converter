...
| ID | Test | Pass condition |
|---|---|---|
| T-01 | LIFT state and motor/preload physical capability | Every applicable T-01A through T-01G sub-test below is recorded. The chosen LIFT position, command limits, and preload range stay inside the measured mechanical and electrical envelope; retracts repeatably without a fault, hard-stop contact, unacceptable drift, or an uncommanded pen contact. |
...
until T-01F identifies a safe working envelope and T-01G establishes a repeatable
LIFT reference.
