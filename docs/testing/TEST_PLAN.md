...
| **T-01C — static motor hold against preload** | After E-06 and E-15, command the motor to the worst intended opposing-spring position, not a hard stop. At nominal 6 V, record actuator position, pen force, rail voltage, motor/driver current, and motor/driver/regulator temperature at the start and after a defined hold dwell. Repeat once with the driver disabled only if the intended LIFT/HOLD design allows it, to determine back-drive or creep. | The owner’s 10 repeated approximately 30 s holds at 6.0 V and 0.18 A provide functional preload-hold evidence. The formal T-01C acceptance still requires the defined position/force/rail/temperature record and drift check if those limits are needed for production qualification. |
