...
| E-09C | Calibrate installed CS1238/load-cell with known precision masses | Partial — the cap-free 2026-09-22 loading/unloading 0–90 g repeat (20 captures) produced `R²=0.999978`, `0.132 g` RMS residual, and a 5,038.77 raw/g downward-weight scale. It supersedes the earlier cap-included run for staged candidates. E-09E confirms that installed upward pen force has the selected opposite raw direction and reaches the intended approximate 40–60 g range. | Partial | The cap-free precision-mass result is the calibration authority; its raw slope and staged thresholds are not refit from kitchen-scale readings. E-09E uses the GP20/GP21 3.3 V USB-to-TTL adapter with adapter VCC disconnected. Arduino IDE Serial Monitor at 115200 supplies bounded 5–100 ms manual pulses; it proves installed direction/range, actuator response resolution, and clear return only. Start at 5 ms. The staged 57 ms (about 1 mm) M5 clearance pulse remains unvalidated until T-01H measures its actual gap and succeeds in 30 cycles. Proceed with bounded controller-dynamics and pen-clear tests using the precision-weight raw profile; kitchen-scale checks need only confirm the practical 40–60 g range. Pico 2, INA101KU, and the 5 N reference fixture are not used. |
| E-09F | Guarded CS1238 force-hold and pen-clear qualification | `s` reaches/holds the precision-weight 40–60 g raw band for five seconds without fault, excess pulse budget, timeout, or implausible scale motion; separate `c` reaches the 3 g clear band then proves the 100 ms candidate air-gap motion. | Partial — initial visual one-cycle clear; repeat protocol reset | Two `t`/`a`/`s` runs reported stable 40.0 g and 40.4 g scale values, passing the requested practical 40–60 g band check. The subsequent extended trace showed several invalid cycles because `t` was sent after `HOLD_COMPLETE` while the pen was still loaded. A loaded tare makes all following deltas relative to contact rather than zero force; those hold/clear outputs cannot qualify force or clearance. That trace also ended with a bounded `clear_up_pulse_budget` fault, so it does not establish repeatability. Restart each cycle only after a visibly clear, stationary pen; never tare between `HOLD_COMPLETE` and `c`. Runtime is the existing GP20/GP21 3.3 V service adapter with VCC disconnected. This temporary bench test does not enable M3/M5, GP27, or production drawing; T-01H retains its repeated-clear-cycle acceptance requirement. |
...
| E-18 | Verify Pro Micro RP2350/TMAG5273 magnetic interface | Installed active-low Aux0/U2/GP28 and GP27/U3-to-PRB transitions with real-magnet A capture | Passed — integrated firmware | The 2026-09-10 motor-inert diagnostic passed arm/release/re-arm, local TMAG `detected=0→1→0`, and the actual GP27/U3-to-PRB state path. Real-magnet A `G38.3`/`.5` captures each returned `:1`; the blue return is at `PROBE SIG`. J1.4 measured 9.33 V released and 0.15 mV asserted relative to `CTRL_GND`. The 2026-09-24 run then exercised the same interface on the production integrated build (`commission=[dir:1 pressure:1 lift:0 mag:1]`): every `M65`/`M64` pair cycled `DISARMED→READY_ACK→WAIT_REARM→SCAN_ACTIVE→DISARMED`, and TMAG `delta` crossed `0.257→3.117→30.287→41.853→0.019` mT with the `MAG_DETECTED` status bit set then clear. See `docs/report/lab-notes/2026-09-10-e-18-motor-inert-p100-handshake.md` and `docs/report/lab-notes/2026-09-24-e-18-m-08-p113-integrated-registration.md`. |
...
| E-14B | Inspect the completed toolhead perfboard before its first 6 V connection | With no supply connected: continuity confirms GP4→IN1, GP5→IN2, GP6→`EEP` sleep input, and `ULT` fault output→GP7; no short exists across the incoming 6 V JST or the 5 V/3.3 V rails to ground; `CTRL_GND` remains isolated from tool ground | Partial | Continuity evidence exists, but the previous `EEP`/`ULT` role interpretation was reversed. Owner confirmed the installed board labels on 2026-09-13. Keep wires in place, reflash the corrected map, and repeat the guarded output test. |
| E-14C | Inspect and function-check ACEIRMC DRV8833 control labels | Retain GP6→`EEP` (sleep input) and `ULT` (protection/fault output)→GP7; firmware drives `EEP` HIGH to enable and reads `ULT` with `INPUT_PULLUP`; J2 state is recorded | Partial | Owner-confirmed labels corrected the earlier role reversal. Reflashed E07B `v` mode measured GP4=3.3 V, GP5=0 V, GP6/EEP=3.3 V relative to driver GND. With N20 leads removed, `o` mode produced both expected OUT1/OUT2 polarities. The driver passes; repair/re-terminate the physical N20 output pair, then repeat a guarded pulse before T-01. |
...
1. **E-07C, power-off / motor-inert bring-up.** Photograph both boards, record
   ...
   board fit and meter both logic lines to ground before powering. With the
   load cell connected, measure and record bridge resistance and `E+`--`E-`
   excitation; inspect the received board's reference circuit before applying
   any published board-specific modification. Flash
   `firmware/pen_pressure/e07c_cs1238_sensor_bringup`, a read-only CS1238
   sketch that leaves GP2 and GP4--GP7 inactive.
...
| F-05 | Spindle/tool output test | Passed (direction and fail-safe) — the RP23CNC spindle `ENA` output is active-high by default, which drove the toolhead's active-low optocoupler input on at idle and made the pen dive at power-up. With `$16=1` (invert spindle enable) `ENA` sits high at idle and `M3` pulls it low, so through the PC817C interface `M3` drives the pen down and `M5` lifts it. The pen-up fail-safe holds with the controller unpowered. `ENA` meter levels at idle/M3/M5 are not yet recorded; `F-05A` remains open. See 2026-09-23 F-05 lab note. |
| F-05A | Validate commissioned GP27/PRB pen-transition acknowledgement | `P115 Q0` accepts a verified initial clear indication; after each M3/M5 command `P115 Q1` observes GP27/PRB inactive then active before the next motion block. A held high, held low, or delayed signal reports `error[39]`; P115 itself issues no motion, M3/M5, Aux0, or actuator command. | In progress | The prerequisites (F-05, E-09C, T-01H) are accepted and `GP27_NORMAL_STATUS_ENABLED` is enabled, so the macro can now be validated against the real toolhead. Remaining: install `P115.macro`, tick the converter box, and run the on-bench `P115 Q0`/`Q1` check; a stuck signal errors `39` rather than running away. |
...
| F-08 | Motorless RP23CNC `PRB`/G38/macro feasibility | Passed — 2026-09-10 direct and actual GP27/U3 paths captured A entry/release with `G38.3`/`G38.5`; the blue return moved from `LIMA` to `PRB`. The 2026-09-24 `G65 P113` runs then verified the remaining items: filesystem `G65 P100`, `#5064` A-probe parameter semantics, coordinate registration, and the 20 ms GP27-inactive interval (`MAG_READY_ACK_DELAY_MS`). |
...
| M-08 | Magnetic bed-center centroid raster | After X/Y physical homing, P100 produces multiple valid equal-pitch X chords, rejects malformed footprints, calculates the chord-width-weighted area centroid, performs the Centroid Approach and Registration Pass, and repeatably sets G54 X0/Y0. 2026-09-24: the 21-row 5 mm raster on the production integrated build completed with no rejected validation and wrote G54 `-232.136,-189.980` from a centroid at `MPos:-232.138,-219.475`; X reproduced the 2026-09-11 centroid to 0.013 mm, and the operator confirmed the parked pen tip perfectly centered over the center magnet. |
| M-09 | Magnetic theta-index registration | At the measured outer radius, P100 finds two outer-magnet entry/exit pairs from one direction, validates spacing near `4320` A motor degrees, approaches the equivalent averaged index, and repeatably sets G54 A0. 2026-09-24: footprints `1264.951/1369.576` and `5597.106/5701.281` gave `4331.930` A motor degrees of spacing and wrote G54 A0 at `5649.193`. The gate consumed 79.5% of its `+/- 15` degree budget and is tracked as a roadmap follow-up. |
| M-10 | Full startup home and registration | One ioSender `G65 P113` command clears the pen, homes physical X/Y, runs P100 Q0 to register center and A, returns to G54 X0 Y0 A0, and leaves the pen lifted. 2026-09-24: completed unattended on the production integrated firmware through `P113 HOME + REGISTER wrapper complete`, parking at `MPos:-232.138,-189.975,0.000,5649.306`. |
...
| T-03 | Force hold | After E-06, E-07, E-08, T-01, and T-02: a bounded moving-average pulse controller holds the calibrated 25–45 g target band through (a) stationary contact, (b) X/Y translation, and (c) progressively faster constant A rotation. It may use only individual 5 ms corrections, after three same-direction 25 ms-separated out-of-band observations and at most once per 250 ms in either direction, except that more than 15 g of excess force triggers a bounded continuous retract-only UP relief of at most 200 ms that stops at the band edge. Log mean, 95th-percentile absolute error, peak force, pulse count/reversals, and faults. No sustained limit cycle, hard-force trip, or uncommanded contact loss is allowed. Demonstrate that the dominant bed-rotation disturbance is within the measured loop bandwidth; otherwise reduce speed or add mechanical compliance before considering feed-forward. |
...
| **T-01H — M5 release and clearance pulse** | With E-07 force calibration active and a scale/paper fixture under the pen, start from stable contact and command normal M5. Record the signed filtered force trace, `F_contact_on`, `F_release_off`, release debounce, retract command, the elapsed release-to-pulse boundary, extra clearance-pulse PWM/duration, the stopped 300 ms settle plus fresh 64-sample clear-state tare, pen-tip gap after stopping, and any mark/drag during a representative pen-up travel move. The staged source uses a 57 ms (about 1 mm) post-release pulse, selected from observed E-09E motor speed; do not accept that value without measurement. Repeat at least 30 M3-contact/M5-clear cycles. `LIFT_HOME` switch contact is not expected during this test. | `F_contact_on` and `F_release_off` have a measured hysteresis margin; release is detected repeatably before the pulse; every non-GP2 cycle completes its fresh clear-state tare; the calibrated pulse leaves the pen clear throughout the representative travel without contacting the distant switch; all 30 cycles complete without fault, drag, or uncommanded paper contact. These values authorize normal high-cycle M5 `PEN_CLEAR` behavior. |
...
**T-01H partial result (2026-09-23):** with the pen installed and paper under
the tip, repeated `M3`/`M5` cycles were commanded through ioSender (about
10-20), jogging X/Y a few mm between each `M5` and the next `M3`. Every `M5`
left the pen clear enough that the inter-stroke jog made no mark, every `M3`
returned to contact, and no cycle faulted. This validates the staged 57 ms
clearance behavior for the supervised bench build. Still open for a full
T-01H: the measured pen-tip gap, the force trace and `F_contact_on` /
