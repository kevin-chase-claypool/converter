...
| E-09F | Guarded CS1238 force-hold and pen-clear qualification | `s` reaches/holds the precision-weight 40–60 g raw band for five seconds without fault, excess pulse budget, timeout, or implausible scale motion; separate `c` reaches the 3 g clear band then proves the 100 ms candidate air-gap motion. | Planned | Flash E-09F by USB-C only with external toolhead power disconnected; runtime is the existing GP20/GP21 3.3 V service adapter with VCC disconnected. Start clear: `t`, `a`, `s`; use the physical cutoff or `x` on any unexpected motion. Record scale range, serial output, correction counts, and faults. Then `a`, `c` and inspect tip clearance without reaching GP2 LIFT_HOME. This one-cycle bench test does not enable M3/M5, GP27, or production drawing; T-01H retains its repeated-clear-cycle acceptance requirement. |
